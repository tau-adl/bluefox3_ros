    ...
    if (value < prop.getMinValue()) {
        return prop.getMinValue();
    }
      if (value > prop.getMaxValue()) {
    ...
    void Bluefox3::onInit()
    {
        ...
        //getParamCheck(nh, "acquire/exposure/AECMode", acq_autoexp_mode, std::string("Off"));
        ...
        m_GenICamACQ_ptr->triggerSelector.writeS( acq_trigger_select );
        m_GenICamACQ_ptr->triggerSource.writeS( acq_trigger_source );
        m_GenICamACQ_ptr->triggerMode.writeS( acq_trigger_enable );
        m_GenICamACQ_ptr->exposureAuto.writeS(acq_autoexp_mode);
        m_GenICamACQ_ptr->exposureTime.writeS(acq_exposure_time);
        ...
        m_GenICamImageFormat_ptr -> pixelFormat.writeS("BayerGR8");
        m_GenICamImageFormat_ptr -> height.writeS("720");
        m_GenICamImageFormat_ptr -> width.writeS("960");

        m_destinationFormat_ptr = std::make_shared<ImageDestination>(m_cameraDevice);
        m_destinationFormat_ptr -> pixelFormat.writeS("Mono8");
